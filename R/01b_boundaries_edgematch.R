...
for (i in code_list) {
  ...
  em <- mutate(points, geom = voron[unlist(st_intersects(points, voron))]) |>
    ...
    left_join(st_drop_geometry(labels))
  
  drop <- filter(labels, !OBJECTID %in% em$OBJECTID)
  ...
  if (nrow(afw_em) == 0) {
    ...
    afw_em <- bind_rows(afw_em, em)
    afw_dropped <- bind_rows(afw_dropped, drop)
  }
}
...
# check valid
all(st_is_valid(afw_em))

# check dropped regions, small islands or peninsulas not in WB admin0?
afw_dropped$code
  # GMB: 7 admin3 (Banjul City Council)
  # GNQ: 1 admin2 island "Corisco" (Litoral)
  # SLE - 4 admin3 islands/peninsulas "Dema" (Southern Province), 
          # "York Rural", "West III", "Tasso Island" (Western Area)

  # add dropped regions back in (missing from WB admin0 but not disputed)
  afw_em <- bind_rows(afw_em, st_make_valid(afw_dropped))

# standardize names
afw_em_clean <- afw_em |>
  mutate(
    adm0_name = if_else(!is.na(ADM0_EN), ADM0_EN,
  ...
  select(
    geo_code, code, adm0_pcode, adm0_name, adm1_pcode, adm1_name,
...
# ADMIN 2
afw_em_admin2 <- group_by(
  afw_em_clean, code, adm0_pcode, adm0_name,
...
library(openxlsx)
afw_1 <- st_read("1_data/Maps/boundaries/AFW_admin1.gpkg")
afw_2 <- st_read("1_data/Maps/boundaries/AFW_admin2.gpkg")
afw_x <- st_read("1_data/Maps/boundaries/AFW_adminX.gpkg")

xl_lst <- list(
  "AFW_admin1" = st_drop_geometry(afw_1),
  "AFW_admin2" = st_drop_geometry(afw_2),
  "AFW_adminX" = st_drop_geometry(afw_x)
)
write.xlsx(xl_lst, "1_data/Maps/boundaries/AFW_admin_regions.xlsx")

#------------------------------------------------------------------------------#
# checks
#------------------------------------------------------------------------------#

# #check and visualize coverage
# library(leaflet)
#
# map <- select(afw_x,code,geo_code)
#
# leaflet(map) |>
...
#
# length(unique(afw_x$geo_code))
# length(unique(afw_x$code))
